...
import { AVATARS, CPU_EMOJI, UnoService } from '../game/uno.service';
import { MAX_PLAYERS, type UnoLevel, type UnoMode } from '../game/uno.types';
...
@Component({
  ...
  templateUrl: './uno-home.component.html',
  styleUrl: './uno-home.component.css',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class UnoHomeComponent implements OnInit {
  ...
  protected readonly mode = signal<UnoMode>('local');
  protected readonly level = signal<UnoLevel>('medium');
  /** Mitspieler am Gerät – Menschen wie Computer, in Sitzreihenfolge. */
  ...
  private cpuName(index: number): string {
    return `${this.translate.instant('gimmicks.games.unoGame.computer')} ${index}`;
  }

  protected onMode(value: UnoMode): void {
    this.mode.set(value);
  ...
  protected startLocal(): void {
    ...
    const seats = this.seats().map((s, i) => ({
      ...s,
      name:
        s.name.trim() || this.translate.instant('gimmicks.games.unoGame.playerNr', { nr: i + 1 }),
    }));
