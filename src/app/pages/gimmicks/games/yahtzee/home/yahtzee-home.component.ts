...
import { AVATARS, CPU_EMOJI, MAX_PLAYERS, YahtzeeService } from '../game/yahtzee.service';
import type { YLevel, YMode } from '../game/yahtzee.types';
...
@Component({
  selector: 'app-yahtzee-home',
  imports: [CommonModule, FormsModule, TranslateModule, IonSegment, IonSegmentButton, IonLabel],
  templateUrl: './yahtzee-home.component.html',
  styleUrl: './yahtzee-home.component.css',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class YahtzeeHomeComponent implements OnInit {
  ...
  protected readonly mode = signal<YMode>('local');
  protected readonly level = signal<YLevel>('medium');
  ...
  private cpuName(index: number): string {
    return `${this.translate.instant('gimmicks.games.yahtzeeGame.computer')} ${index}`;
  }

  protected onMode(value: YMode): void {
    this.mode.set(value);
  ...
  protected startLocal(): void {
    ...
    const seats = this.seats().map((s, i) => ({
      ...s,
      name: s.name.trim() || this.translate.instant('gimmicks.games.yahtzeeGame.playerNr', { nr: i + 1 }),
    }));
